...
async fn test_admin_streams_route_present_when_enabled() {
    ...
    config.admin.base_path = "/ops".to_string();
    ...
    let response = client
        .get(format!("{base_url}/ops/streams"))
        .send()
        .await
    ...
    let default_admin_response = client
        .get(format!("{base_url}/admin/streams"))
        .send()
        .await
        .expect("default admin path request failed");
    assert_eq!(default_admin_response.status(), 404);
